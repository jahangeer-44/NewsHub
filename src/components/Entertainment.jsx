...
const Entertainment = () => { const [newsData, setNewsData] = useState([]);

  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await fetch(
          `https://newsapi.org/v2/everything?q=Entertainment&apiKey=${API_KEY}`
        );
  ...
  return (
    <div>
      <h2 className="text-center text-3xl font-bold mt-5">Entertainment News</h2>
